...
# H0: performance is significantly different from zero
# H1: performance is not significanlty different from zero
def p_val(
    y_true,
    y_pred_1,
    y_pred_2,
    ...
    for idx_vals in indices:
        ...
        if reject_one_class_samples and len(np.unique(y_true[idx_vals])) < 2:
            continue
        score_1 = calculate_roc_score(y_true, y_pred_1, idx_vals, sample_weight)
        score_2 = calculate_roc_score(y_true, y_pred_2, idx_vals, sample_weight)
